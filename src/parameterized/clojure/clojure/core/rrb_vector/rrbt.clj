    ...
    (if (zero? shift)
      ...
      (let [regular?  (.regular nm node)
            ...
            new-child (slice-right nm am (aget ^objects arr i)
                                   (unchecked-subtract-int shift (int p/shift-increment))
                                   child-end)
            regular-child? (if (== shift (int p/shift-increment))
                             (== (int p/max-branches) (.alength am (.array nm new-child)))
                             (.regular nm new-child))
            new-arr   (object-array (if (and regular? regular-child?) p/max-branches p/non-regular-array-len))
            new-child-rng  (if regular-child?
                             (let [m (mod child-end (bit-shift-left 1 shift))]
                               (if (zero? m) (bit-shift-left 1 shift) m))
                             (if (== shift (int p/shift-increment))
        ...
        (if-not (and regular? regular-child?)
          (let [new-rngs (int-array p/non-regular-array-len)
            ...
            (aset new-rngs i (unchecked-add-int
                              (if (pos? i)
                                ...
                                (int 0))
                              new-child-rng))
            (aset new-rngs p/max-branches (unchecked-inc-int i))
  ...
  (let [shift (int shift)
    ...
    (if (zero? shift)
      ...
      (let [regular? (.regular nm node)
            arr      (.array nm node)
            rngs     (if-not regular? (ranges nm node))
            i        (bit-and (bit-shift-right start shift) (int p/branch-mask))
            i        (if regular?
                       i
                       (loop [j i]
            ...
            len      (if regular?
                       (loop [i i]
                         (if (or (== i (int p/max-branches))
                                 (nil? (aget ^objects arr i)))
                           i
                           (recur (unchecked-inc-int i))))
                       (aget rngs p/max-branches))
        ...
        (cond
          ...
          (let [new-arr (object-array p/non-regular-array-len)
                ...
                rng0    (if (or (nil? new-child)
                                (== shift (int p/shift-increment))
                                (.regular nm new-child))
                          (unchecked-subtract-int
                           (bit-shift-left (int 1) shift)
                           (bit-and (bit-shift-right
                                     start (unchecked-subtract-int shift (int p/shift-increment)))
                                    (int p/branch-mask)))
          ...
          (let [new-arr  (object-array p/non-regular-array-len)
            ...
            (aset new-rngs p/max-branches new-len)
            (System/arraycopy arr (if (nil? new-child) (unchecked-inc-int i) i)
                              new-arr 0
                              new-len)
            (if-not (nil? new-child)
...
(deftype Vector [^NodeManager nm ^ArrayManager am ^int cnt ^int shift root tail
      ...
      (let [tail-off (unchecked-subtract-int cnt (.alength am tail))]
        (if (<= tail-off i)
          ...
          (loop [i i node root shift shift]
            (if (zero? shift)
              (let [arr (.array nm node)]
                (.aget am arr (bit-and (bit-shift-right i shift) (int p/branch-mask))))
              (if (.regular nm node)
                (let [arr (.array nm node)
                      idx (bit-and (bit-shift-right i shift) (int p/branch-mask))]
                  (loop [i     i
                         node  (aget ^objects arr idx)
                         shift (unchecked-subtract-int shift (int p/shift-increment))]
                    (let [arr (.array nm node)
                          idx (bit-and (bit-shift-right i shift) (int p/branch-mask))]
                      (if (zero? shift)
                        (.aget am arr idx)
                ...
                (let [arr  (.array nm node)
                      rngs (ranges nm node)
                      idx  (loop [j (bit-and (bit-shift-right i shift) (int p/branch-mask))]
                             (if (< i (aget rngs j))
                               j
                               (recur (unchecked-inc-int j))))
                      i    (if (zero? idx)
  ...
  (cons [this val]
    (if (< (.alength am tail) (int p/max-branches))
      ...
      (let [tail-node (.node nm (.edit nm root) tail)
        ...
        (if (overflow? nm root shift cnt)
          (if (.regular nm root)
            (let [new-arr  (object-array p/max-branches)
              ...
              (Vector. nm am
                       ...
                       (unchecked-add-int shift (int p/shift-increment))
                       new-root new-tail _meta 0 0))
            (let [new-arr  (object-array p/non-regular-array-len)
                  new-rngs (ints (int-array p/non-regular-array-len))
                  new-root (.node nm (.edit nm root) new-arr)
                  root-total-range (aget (ranges nm root) (int p/max-branches-minus-1))]
              (doto new-arr
                (aset (int 0)  root)
                (aset (int 1)  (.newPath this (.edit nm root) shift tail-node))
                (aset (int p/max-branches) new-rngs))
              (doto new-rngs
                (aset (int 0)  root-total-range)
                (aset (int 1)  (unchecked-add-int root-total-range (int p/max-branches)))
                (aset (int p/max-branches) (int 2)))
              (Vector. nm am
                       (unchecked-inc-int cnt)
                       (unchecked-add-int shift (int p/shift-increment))
                       new-root new-tail _meta 0 0)))
  ...
  (empty [_]
    (Vector. nm am 0 p/shift-increment (.empty nm) (.array am 0) _meta 0 0))
    ...
    (cond
      ...
      (let [new-tail (.arrayFor this (unchecked-subtract-int cnt (int 2)))
        ...
        (cond
          ...
          (and (> shift (int p/shift-increment))
          ...
          (if (zero? shift)
            ...
            (if (.regular nm node)
              (loop [node  (aget ^objects (.array nm node)
                                 (bit-and (bit-shift-right i shift) (int p/branch-mask)))
                     shift (unchecked-subtract-int shift (int p/shift-increment))]
                (if (zero? shift)
                  (.array nm node)
              ...
              (let [rngs (ranges nm node)
                    j    (loop [j (bit-and (bit-shift-right i shift) (int p/branch-mask))]
                           (if (< i (aget rngs j))
                             j
                             (recur (unchecked-inc-int j))))
                    i    (if (pos? j)
  ...
  (pushTail [this shift cnt node tail-node]
    (if (.regular nm node)
            ...
            (if (== shift (int p/shift-increment))
              ...
              (if-let [child (aget ^objects arr subidx)]
                (let [new-carr  (aclone ^objects (.array nm child))
                  ...
                  (recur new-child (unchecked-subtract-int shift (int p/shift-increment))))
      ...
      (let [arr  (aclone ^objects (.array nm node))
            ...
            ret  (.node nm (.edit nm node) arr)
            cret (if (== shift (int p/shift-increment))
                   nil
        ...
        (if cret
          (do (aset ^objects arr li cret)
              (aset rngs li (unchecked-add-int (aget rngs li) (int p/max-branches)))
              ret)
          (do (when (>= li p/max-branches-minus-1)
              ...
              (aset rngs (unchecked-inc-int li)
                    (unchecked-add-int (aget rngs li) (int p/max-branches)))
  ...
  (popTail [this shift cnt node]
    (if (.regular nm node)
      (let [subidx (bit-and
                    ...
                    (int p/branch-mask))]
        (cond
          (> (int shift) (int p/shift-increment))
      ...
      (let [rngs   (ranges nm node)
        ...
        (cond
            ...
            (cond
              ...
              (let [arr (aclone ^objects (.array nm node))]
                (aset new-rngs subidx
                      (unchecked-subtract-int (aget new-rngs subidx) (int p/max-branches)))
                (aset arr subidx new-child)
                (aset arr (int p/max-branches) new-rngs)
                (if (nil? new-child)
                  (aset new-rngs p/max-branches (unchecked-dec-int (aget new-rngs p/max-branches))))
                (.node nm (.edit nm root) arr))
          ...
          (let [arr      (aclone ^objects (.array nm node))
            ...
            (aset new-rngs p/max-branches (unchecked-dec-int (aget new-rngs (int p/max-branches))))
            (.node nm (.edit nm root) arr))))))
        ...
        (loop [s (int 0) node node]
          (if (== s shift)
            ...
            (let [arr (object-array p/max-branches)
              ...
              (aset arr 0 node)
              (recur (unchecked-add-int s (int p/shift-increment)) ret)))))
      (let [shift (int shift)]
        (loop [s (int 0) node node]
          (if (== s shift)
            node
    ...
    (if (.regular nm node)
      (let [node (.clone nm am shift node)]
        (loop [shift (int shift)
          ...
          (if (zero? shift)
            (let [arr (.array nm node)]
              (.aset am arr (bit-and i (int p/branch-mask)) val))
            (let [arr    (.array nm node)
                  subidx (bit-and (bit-shift-right i shift) (int p/branch-mask))
                  next-shift (int (unchecked-subtract-int shift (int p/shift-increment)))
          ...
          (if (>= start tail-off)
            (let [new-tail (.array am new-cnt)]
              (System/arraycopy tail (unchecked-subtract-int start tail-off)
                                new-tail 0
                                new-cnt)
              (Vector. nm am new-cnt (int p/shift-increment) (.empty nm) new-tail _meta 0 0))
            (let [tail-cut? (> end tail-off)
              ...
              (if (nil? new-root)
                (Vector. nm am new-cnt p/shift-increment (.empty nm) new-tail _meta 0 0)
                (loop [r new-root
                       s (int shift)]
                  (if (and (> s (int p/shift-increment))
                           (nil? (aget ^objects (.array nm r) 1)))
                    (recur (aget ^objects (.array nm r) 0)
                           (unchecked-subtract-int s (int p/shift-increment)))
...
(defn shift-from-to [^NodeManager nm node from to]
  (cond
    ...
    (.regular nm node)
    (recur nm
           (.node nm (.edit nm node) (doto (object-array p/max-branches) (aset 0 node)))
           (unchecked-add-int (int p/shift-increment) (int from))
           to)
    ...
    (recur nm
           (.node nm
                  (.edit nm node)
                  (doto (object-array p/non-regular-array-len)
                    (aset 0 node)
                    (aset p/max-branches
                          (ints (doto (int-array p/non-regular-array-len)
                                  (aset 0  (int (last-range nm node)))
                                  (aset p/max-branches (int 1)))))))
           (unchecked-add-int (int p/shift-increment) (int from))
           to)))
      ...
      (loop [i 0 sbc 0]
        (if (== i p/max-branches)
          ...
          (if-let [child (aget ^objects arr i)]
            ...
            sbc)))
      (let [lim (aget (ranges nm node) p/max-branches)]
        (loop [i 0 sbc 0]
          (if (== i lim)
            sbc
...
(defn rebalance-leaves
  ...
  (let [slc1 (slot-count nm am n1 p/shift-increment)
        slc2 (slot-count nm am n2 p/shift-increment)
        a    (+ slc1 slc2)
        sbc1 (subtree-branch-count nm am n1 p/shift-increment)
        sbc2 (subtree-branch-count nm am n2 p/shift-increment)
        p    (+ sbc1 sbc2)
    ...
    (cond
      ...
      (let [reg?    (zero? (mod p p/max-branches))
        ...
        (loop [i  0
          ...
          (when-first [block bs]
            (let [a (.array am (count block))]
              ...
              (recur (inc i) (next bs)))))
        (if-not reg?
          (aset new-arr p/max-branches (regular-ranges p/shift-increment p)))
        (set! (.-val transferred-leaves) sbc2)
        (pair new-n1 nil))

      :else
      (let [reg?     (zero? (mod p p/max-branches))
            new-arr1 (object-array p/max-branches)
            new-arr2 (object-array (if reg? p/max-branches p/non-regular-array-len))
          ...
          (when-first [block bs]
            (let [a (.array am (count block))]
              ...
              (if (< i p/max-branches)
                ...
                (aset new-arr2 (- i p/max-branches) (.node nm nil a)))
              (recur (inc i) (next bs)))))
        (if-not reg?
          (aset new-arr2 p/max-branches (regular-ranges p/shift-increment (- p p/max-branches-squared))))
        (set! (.-val transferred-leaves) (- p/max-branches-squared sbc1))
        (pair new-n1 new-n2)))))
...
(defn child-seq [^NodeManager nm node shift cnt]
  (let [arr  (.array nm node)
        rngs (if (.regular nm node)
               ...
               (ranges nm node))
        cs   (if rngs (aget rngs p/max-branches) (index-of-nil arr))
        cseq (fn cseq [c r]
               (let [arr  (.array nm c)
                     rngs (if (.regular nm c)
                            (ints (regular-ranges (- shift p/shift-increment) r))
                            (ranges nm c))
                     gcs  (if rngs (aget rngs p/max-branches) (index-of-nil arr))]
      ...
      (cond
        ...
        (let [new-arr  (object-array p/non-regular-array-len)
          ...
          (loop [i  0
            ...
            (when-first [block bs]
              (let [a (object-array p/non-regular-array-len)
                    r (int-array p/non-regular-array-len)]
                (aset a p/max-branches r)
                (aset r p/max-branches (count block))
                ...
                (aset new-rngs i
                      (+ (aget r (dec (aget r p/max-branches)))
        ...
        :else
        (let [new-arr1  (object-array p/non-regular-array-len)
              new-arr2  (object-array p/non-regular-array-len)
              new-rngs1 (int-array p/non-regular-array-len)
              ...
              (let [a (object-array p/non-regular-array-len)
                ...
                (loop [i 0 o (int 0) gcs (seq block)]
                  (when-first [[gc gcr] gcs]
                    ...
                    (aset r i (unchecked-add-int o (int gcr)))
                    (recur (inc i) (unchecked-add-int o (int gcr)) (next gcs))))
                (if (and (< i p/max-branches) (> (+ (* i p/max-branches) (count block)) sbc1))
                  (let [tbs (- (+ (* i p/max-branches) (count block)) sbc1)
                        li  (dec (aget r p/max-branches))
                ...
                (let [new-arr  (if (< i p/max-branches) new-arr1 new-arr2)
                      new-rngs (if (< i p/max-branches) new-rngs1 new-rngs2)
                      i        (mod i p/max-branches)]
                  (aset ^objects new-arr i (.node nm nil a))
                  (aset new-rngs i
                        (+ (aget r (dec (aget r p/max-branches)))
                           (if (pos? i) (aget new-rngs (dec i)) (int 0))))
                  (aset new-rngs p/max-branches (int (inc i))))
                (recur (inc i) (next bs)))))
          (aset new-arr1 p/max-branches new-rngs1)
      ...
      (let [new-rngs (int-array p/non-regular-array-len)
        ...
        (loop [i 0 rngs (seq rngs)]
          (if rngs
            ...
            (aset new-rngs p/max-branches i)))
...
;; (take (inc p/max-branches)) is just a technique to avoid generating more
;; grandchildren than necessary.  If there are at least (inc p/max-branches), we do not
;; care how many there are.
#_(defn bounded-grandchildren [nm children]
  (into [] (comp (map #(child-nodes % nm))
                 cat
                 (take (inc p/max-branches)))
        children))
...
;; A regular tree node is guaranteed to have only p/max-branches-way branching at
...
;; full p/max-branches vector elements.  This code relies on these invariants to
  ...
  (if (.regular nm node)
    (loop [node node
      ...
      (if (zero? shift)
        (if (nil? node)
          acc
          ;; The +p/max-branches is for the regular leaf node reached at shift 0
          (+ acc p/max-branches))
  ...
  (let [config @peephole-optimization-config]
    (if (<= (.-shift v) (* 2 p/shift-increment))
      ...
      (let [root (.-root v)
            ...
            many-grandchildren? (> num-granchildren-bounded p/max-branches)]
        (if many-grandchildren?
          ...
          (let [^objects new-arr  (object-array p/non-regular-array-len)
                ^ints new-rngs (int-array p/non-regular-array-len)
                new-root (.node nm (.edit nm root) new-arr)
                shift    (.-shift v)
                grandchild-shift (- shift (* 2 p/shift-increment))]
            ...
            (aset new-rngs p/max-branches num-granchildren-bounded)
            (aset new-arr p/max-branches new-rngs)
            (let [new-v (Vector. nm (.-am v) (.-cnt v) (- shift p/shift-increment)
    ...
    (let [s1 (.-shift v1)
          ...
          r1 (if o?
               (let [tail      (.-tail v1)
                     ...
                     reg?      (and (.regular nm r1) (== (.alength am tail) p/max-branches))
                     arr       (object-array (if reg? p/max-branches p/non-regular-array-len))]
                 (aset arr 0 r1)
                 (aset arr 1 (new-path nm am s1 tail-node))
                 (if-not reg?
                   (let [rngs (int-array p/non-regular-array-len)]
                     (aset rngs p/max-branches 2)
                     (aset rngs 0 (- (count v1) (.alength am tail)))
                     (aset rngs 1 (count v1))
                     (aset arr p/max-branches rngs)))
                 (.node nm nil arr))
               (fold-tail nm am r1 s1 (.tailoff v1) (.-tail v1)))
          s1 (if o? (+ s1 p/shift-increment) s1)
          r2 (.-root v2)
          s  (max s1 s2)
      ...
      (post-splice-fixup
       ...
       (if n2
         (let [arr      (object-array p/non-regular-array-len)
           ...
           (aset arr 0 n1)
           (aset arr 1 n2)
           (aset arr p/max-branches (doto (int-array p/non-regular-array-len)
                          (aset 0 ncnt1)
                          (aset 1 (+ ncnt1 ncnt2))
                          (aset p/max-branches 2)))
           (Vector. nm am (+ (count v1) (count v2)) (+ s p/shift-increment) new-root (.-tail v2)
                    nil 0 0))
         (loop [r n1
                s (int s)]
           (if (and (> s (int p/shift-increment))
        ...
        (if (overflow? nm root shift cnt)
          (if (.regular nm root)
            (let [new-arr (object-array p/max-branches)]
              ...
              (set! shift (unchecked-add-int shift (int p/shift-increment)))
              (set! cnt   (unchecked-inc-int cnt))
              this)
            (let [new-arr  (object-array p/non-regular-array-len)
                  new-rngs (int-array p/non-regular-array-len)
                  new-root (.node nm (.edit nm root) new-arr)
                  root-total-range (aget (ranges nm root) p/max-branches-minus-1)]
              (doto new-arr
                (aset 0  root)
                (aset 1  (.newPath transient-helper nm am
                                   tail (.edit nm root) shift tail-node))
                (aset p/max-branches new-rngs))
              (doto new-rngs
                (aset 0  root-total-range)
                (aset 1  (unchecked-add-int root-total-range (int p/max-branches)))
                (aset p/max-branches 2))
              (set! root  new-root)
              (set! shift (unchecked-add-int shift (int p/shift-increment)))
              (set! cnt   (unchecked-inc-int cnt))
    ...
    (cond
      ...
      (let [new-tail-base (.arrayFor this (unchecked-subtract-int cnt (int 2)))
        ...
        (cond
          ...
          (and (> shift p/shift-increment)
               (nil? (aget ^objects (.array nm new-root) 1)))
          (do (set! cnt   (unchecked-dec-int cnt))
              (set! shift (unchecked-subtract-int shift (int p/shift-increment)))
