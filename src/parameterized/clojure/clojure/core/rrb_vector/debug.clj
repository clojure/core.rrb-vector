...
(defn children-summary [node shift get-array get-ranges regular? opts]
  (let [children (get-array node)
        ...
        exp-array-len (if reg? p/max-branches p/non-regular-array-len)
        bad-array-len? (not= array-len exp-array-len)]
    ;; 'r' for regular, 'i' for irregular
    ;; For either type of node, its first p/max-branches array elements are broken
    ;; down into:
    ;; # regular children
    ;; # irregular children

    ;; # of nil 'children' not shown, since it will always be p/max-branches minus
...
(defn dbg-vec
 ([v]
  (dbg-vec v {:max-depth nil   ;; integer to limit depth, nil for unlimited
              ...
              ;; p/max-branches), and to show the 'deltas' between consecutive
                                ...
                                (or visit-all-children?
                                    (and (:always-show-fringes opts)
                                         (or child-on-left-fringe?
                                             child-on-right-fringe?)))]
                            (if visit-this-child?
                              (go (inc indent) (- shift p/shift-increment) i node
...
(defn basic-node-errors [v]
  (let [{:keys [v get-shift]} (pd/unwrap-subvec-accessors-for v)
        ...
        expected-leaf-depth (+ (quot shift p/shift-increment) 2)
        ...
        (cond
          ...
          (> (count v) p/non-regular-array-len) #{(dec expected-leaf-depth)}
          :else #{(dec expected-leaf-depth)
                  (- expected-leaf-depth 2)})]
    (cond
      (not= (mod shift p/shift-increment) 0)
      {:error true
       :description (str "shift value in root must be a multiple of p/shift-increment.  Found "
                         shift)
       :data shift}
...
(defn objects-in-slot-32-of-obj-arrays
  ...
  object, which is often an array of p/max-branches or p/non-regular-array-len java.lang.Object's, has
  an element at index p/max-branches that is not nil, and refers to an object that
  ...
  (let [{:keys [v get-array]} (pd/unwrap-subvec-accessors-for v)
    ...
    (keep (fn [node-info]
            ...
            (let [arr (get-array (:node node-info))
                  n (count arr)]
              (if (== n p/non-regular-array-len)
                (aget arr p/max-branches))))
          internal)))
  ...
  (let [[full-children others] (split-with :full? children)
    ...
    (cond
      ...
      (not (or (and root-node?
                    (<= root-node-cnt p/max-branches)  ;; all elements in tail
                    (= 0 num-non-nil))
               (<= 1 num-non-nil p/max-branches)))
      {:error true, :kind :internal
       :description
       (str "Found internal regular node with # full + # partial=" num-non-nil
            " children outside of range [1, " p/max-branches "]."
            " root-node?=" root-node? " root-node-cnt=" root-node-cnt)
       :data children}
      :else
      {:error false, :kind :internal,
       :full? (= p/max-branches (count full-children))
    ...
    (cond
      ...
      {:error true, :kind :internal,
       :description (str "Found internal non-regular node with "
                         ...
                         (count others) " 'other' children."
                         " - expected 0 children after nils.")}
      (not= num-non-nil (aget rng p/max-branches))
      {:error true, :kind :internal,
       :description (str "Found internal non-regular node with "
                         num-non-nil " non-nil, " num-nil " nil children, and"
                         " last elem of ranges=" (aget rng p/max-branches)
      ...
      (not (<= 1 (aget rng p/max-branches) p/max-branches))
      {:error true, :kind :internal
       :description (str "Found internal non-regular node with (aget rng p/max-branches)"
                         "=" (aget rng p/max-branches) " outside of range [1, p/max-branches].")}
      :else
      {:error false, :kind :internal, :full? false,
       :count (last expected-ranges)})))


(defn max-capacity-divided-by-max-branches-squared [root-shift]
  (let [shift-amount (max 0 (- root-shift p/shift-increment))]
...
(defn fraction-full [v]
  (let [{:keys [v get-shift]} (pd/unwrap-subvec-accessors-for v)
        ...
        max-tree-cap (bit-shift-left 1 (+ root-shift p/shift-increment))]
  ...
  (let [{:keys [v subvector? subvec-start subvec-end
    ...
    (letfn [(go [shift node]
              (if node
                (if (not= shift 0)
                  (apply concat
                         (map (partial go (- shift p/shift-increment))
                              (let [arr (get-array node)]
                                (if (regular? node)
                                  arr
                                  (butlast arr)))))
